    ...
    <div className="w-full animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl mx-auto space-y-6 pt-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <h2 className="text-3xl font-extrabold tracking-tight text-blue-900">Vote Distribution Detail</h2>
          <p className="text-sm text-slate-500 mt-1 font-medium">Detailed breakdown of candidate performance</p>
        </div>
      </div>

      <Card className="shadow-xl shadow-blue-900/5 border border-blue-100 bg-white/95 backdrop-blur-sm overflow-hidden rounded-3xl">
        <CardHeader className="bg-slate-50/50 border-b border-blue-50 py-6 px-8">
          <CardTitle className="text-xl font-bold text-blue-900 flex items-center gap-2">
            Election Results Leaderboard
          </CardTitle>
          <p className="text-sm text-slate-500">Visualization of current vote counts scaled by volume.</p>
        </CardHeader>
              ...
              <div className="flex flex-row items-end justify-center w-full gap-4 md:gap-8 pt-12">
               ...
               return (
                 <div key={candidate.id} className="relative group w-full flex flex-col items-center justify-end h-80">
                    ...
                    <div className="shrink-0 w-20 h-20 rounded-full bg-white p-1 shadow-xl border-4 border-white backdrop-blur-md z-20 relative -mb-6 md:-mb-8 transition-transform group-hover:-translate-y-2">
                      {candidate.imageUrl ? (
                         <>
                      ...
                      <div className="absolute top-0 right-0 -mt-1 -mr-1 w-6 h-6 bg-primary text-white rounded-full flex items-center justify-center font-black text-xs shadow-md border-2 border-white">
                        {candidate.number}
                      </div>
                    </div>
                    ...
                    <div className="text-center mt-3 h-16">
                       <h4 className="font-bold text-slate-800 text-sm md:text-base leading-tight mb-1">{candidate.name}</h4>
                       <span className="font-black text-lg text-blue-900">{candidate.votes} <span className="text-xs font-semibold text-slate-500">Votes</span></span>
                    </div>
                 </div>
                 );
                 })}
